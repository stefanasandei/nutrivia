...
export const adminRouter = createTRPCRouter({
  ...
  addFood: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (ctx.session.user.id != env.ADMIN_ID) isHidden = true;
  ...
  getFoodComments: publicProcedure
    .input(z.object({ id: z.number() }))
    .query(async ({ ctx, input }) => {
      return await ctx.db.foodProduct.findFirst({
        where: { id: input.id },
  ...
  likeFoodProduct: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        input.isLike &&
        !foodPost?.likedBy.includes(input.userId) &&
        !foodPost?.dislikedBy.includes(input.userId)
      ) {
  ...
  agreeRules: protectedProcedure.mutation(async ({ ctx }) => {
  ...
  countUsers: protectedProcedure.query(async ({ ctx }) => {
